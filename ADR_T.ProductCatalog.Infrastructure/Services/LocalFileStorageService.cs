    ...
    public class LocalFileStorageService : IFileStorageService
    {
        ...
        public async Task<string> SaveFileAsync(Stream fileStream, string fileName, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (fileStream == null)
                {
                    throw new ArgumentNullException(nameof(fileStream), "El stream no puede ser nulo.");
                }
            ...
            {
                _logger.LogWarning("Operación cancelada al guardar el archivo: {FileName}", fileName);
                throw;
            }
            catch (Exception ex)
            {
